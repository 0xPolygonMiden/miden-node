...
pub fn write_proto(target_dir: &Path) -> Result<(), String> {
    if !target_dir.exists() {
        fs::create_dir_all(target_dir).map_err(|e| format!("Error creating directory: {}" e.to_string()))?;
    ...
    for (file_name, file_content) in PROTO_FILES {
        ...
        let mut file = File::create(&file_path).map_err(|e| format!("Error creating file: {}" e.to_string()))?;
        file.write_all(file_content.as_bytes()).map_err(|e| format!("Error writing file: {}" e.to_string()))?;
    }
