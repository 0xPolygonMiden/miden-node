...
impl Funder {
    ...
    async fn refresh_status(&mut self) -> Result<()> {
        ...
        self.setup.status.update(self.fee_balance(&funder), block_num);
        Ok(())
    }
    ...
    async fn run_batch(
        ...
        self.setup.status.update(balance, reference_block);
        ...
        let PreparedBatch { proven_tx, transaction_inputs, notes } = self
            .prepare_batch(reference_header, blockchain, funder, fee_faucet, &targets)
            .await
            .map_err(|err| {
